...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Fetch live nearest advisories generated by AWS ML Model from PostGIS
  useEffect(() => {
    async function loadNearestAdvisories() {
      try {
        const lat = userProfile?.lat || 14.0122;
        const lng = userProfile?.lng || 123.0114;
        const res = await fetch(`/api/advisories/nearest?lat=${lat}&lon=${lng}`);
        if (res.ok) {
          const data = await res.json();
          if (data.hotspots && data.hotspots.length > 0) {
            const mapped: Hotspot[] = data.hotspots.map((item: any, idx: number) => ({
              ...
              compassBearing: item.compass_bearing,
              sst: item.sst,
              chlA: item.chl_a
            }));

            setHotspots([...mapped, ...DEFAULT_HOTSPOTS]);
          }
        }
      } catch (err) {
        console.warn("Failed to load AWS advisories, using defaults:", err);
      }
    }
    loadNearestAdvisories();
